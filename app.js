...
5. 答錯時在圖上標出錯誤位置：marks 每個元素的 box 是 [左,上,右,下] 四個 0~1 之間的小數（相對整張圖的寬與高、原點在左上），框住「寫錯的那一段式子」；label 是 ≤8 字短標（如「6-2 應為 4」「符號錯」）。最多 2 個，座標務必對準圖中該式子的實際位置。答對時 marks 為 []。
只回傳 JSON（不要其他文字）：{"read":"辨識出的答案","correct":true或false,"firstError":"哪一步開始錯（答對時為 null）","praise":"具體稱讚（沒有就 null）","habit":"要注意的計算習慣（沒有就 null）","marks":[{"box":[0.10,0.42,0.55,0.52],"label":"6-2 應為 4"}]}`,
  });
...
function qResolve(ok) {
  ...
  const fb = $('#qfb');
  const v = qsess.ai; // AI 批改結果（只有手寫填充題有）：{read,correct,firstError,praise,habit}
  ...
  // ④ 中段：依「這次結果該看什麼」只給一塊，不層層疊
  let mid = '';
  if (!ok) {
    const marked = v && Array.isArray(v.marks) && v.marks.length && qsess.calcImg ? markedImgHTML(qsess.calcImg, v.marks, v.firstError) : '';
    const errLine = !marked && v && v.firstError ? `<p class="badc" style="margin:8px 0 4px"><b>你從這裡跑掉：</b>${escH(v.firstError)}</p>` : '';
    mid = `${marked}${errLine}<div class="one-method"><b>一種最簡單的算法：</b>${rtTxt(q.sol)}</div>`;
  }
  else if (overtime) mid = q.tip ? `<div class="one-method">⚡ <b>更快的做法：</b>${rtTxt(q.tip)}</div>` : '<p class="dim">這題沒有特別捷徑，多做幾次讓步驟變反射就會快。</p>';
  else mid = praiseFor(q, ok, ms, target); // 答對又準時：只給短稱讚，其餘不秀
  // ⑤ 完整解說——永遠收起（答對又準時完全不擋路）；只放上面沒秀的部分，避免重複
  const solInFull = ok ? `<p><b>詳解：</b>${rtTxt(q.sol)}</p>` : ''; // 答錯已在上面用人話講過
  const tipInFull = q.tip && !overtime ? `<p class="tip">💡 <b>快解：</b>${rtTxt(q.tip)}</p>` : ''; // 超時已在上面放快解
  const aiExtra = v && (v.praise || v.habit) ? `<div class="ai-fb">${v.praise ? `<p class="praise">🎉 ${escH(v.praise)}</p>` : ''}${v.habit ? `<p class="warnc">✏️ 習慣提醒：${escH(v.habit)}</p>` : ''}</div>` : '';
  const full = `<details class="sol-detail" ontoggle="if(this.open){const b=this.querySelector('#mlib-box');if(b&&!b.innerHTML)showMethods('${q.topic}',true)}">
      ...
      ${solInFull}${tipInFull}${teachBlock(q.id)}${aiExtra}
